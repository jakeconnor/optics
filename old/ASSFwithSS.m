...
for n=0:step_num
   if n==4; duu2_dt2= (-2*wavespace(n,:)+5*wavespace(n-1,:)-4*wavespace(n-2,:)+wavespace(n-3,:))/(deltaz^2); %first order accurate
   elseif n>4
       duu2_dt2 = (-35*wavespace(n,:)/12 + 26*wavespace(n-1,:)/3 - 19*wavespace(n-2,:)/2 + 14*wavespace(n-3,:)/3 - 11*wavespace(n-4,:)/12)/(deltaz^2);
   end
...
%     selfsteepen=exp((2.*conj(temp).*duu_dt + uu.*duu_dt_star).*deltaz.*ss);
    dispersion=exp(-1i*beta2*duu2_dt2);
    loss=exp(-alpha/2);    %nts: write better comments
    temp = uu.*exp(abs(uu).^2.*hhz).*loss.*dispersion; %<thisone ???
