classdef fdfiber %An object that simulates fibers
    ...
    methods
        function obj = fdfiber(length, beta1, N, alpha )
            ...
            obj.N = N;
            obj.alpha=alpha;
            obj.step_num_z=obj.fiberLength*500*(obj.N);
        ...
        function obj = simulateFiber(obj, Vin, time)
            ...
            for n=1:tstep
                ...
                if any(isnan(obj.uu)|isinf(obj.uu))
                    obj.Vout=NaN;
                    % disp('Simulation Failed, try smaller Z-steps')
                    return; %Vout returning NaN won't prevent the parent simulation from re-calling, but will break the graph
                end
                if any(isnan(obj.uu));
                    return
                end
