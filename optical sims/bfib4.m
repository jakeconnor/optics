classdef bfib4
    ...
    methods
        function obj=bfib4(length,beta1a,na,lossa,beta1b,nb,lossb,sigma)
            ...
            %build our two fibers, calcualte step sizes
            for nfib=1:2
                ...
                obj.uu{nfib,1}=0;%this is weirdly necessary
                [obj.uu{nfib,1:(0.005/obj.deltat{nfib})}]=deal(((1+1i)*zeros(obj.step_num_z{nfib},1)));%%
                ...
                %build a spacing matrix to help interpolate from one size to the other
            ...
            for nfib=1:2
                ...
                obj.interp{nfib}= zeros(size(obj.uu{nfib},1),size(obj.uu{oppfib},1)); 
                %interp{nfib} is multiplied by oppfib to interpolate to the
                %size of nfib
        ...
        function obj = simulateFiber(obj, Vina, Vinb, circstep, ~, circdeltat)
            ...
            while obj.time{1} < circstep*circdeltat || obj.time{2} < circstep*circdeltat
                ...
                obj.uu{nfib,obj.currentstep{nfib}}(1)=Vin{nfib}; %bring in the most current part ofthe input wave
                ...
                obj.uu{nfib,obj.currentstep{nfib}}(:)= obj.uu{nfib,obj.currentstep{nfib}}(:)+((obj.deltat{nfib}/obj.beta1{nfib})* ...
                    ...
                    *(obj.N{nfib}.*(abs(obj.uu{nfib,obj.currentstep{nfib}}(:)).^2)...
                    + obj.sigma.*(abs(obj.interp{nfib}*obj.uu{oppfib,obj.currentstep{oppfib}}(end:-1:1)).^2)).* ...
            ...
            obj.Vouta=(weight{1}*obj.uu{1,obj.currentstep{1}-1}(end)+(1-weight{1})*obj.uu_old{1}(end));
            obj.Voutb=(weight{2}*obj.uu{2,obj.currentstep{2}-1}(end)+(1-weight{2})*obj.uu_old{2}(end));
